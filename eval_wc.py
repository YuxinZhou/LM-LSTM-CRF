...
if __name__ == "__main__":
    ...
    parser.add_argument('--test_file', default='',
                        help='Path to the test file. If set to None, would use the test_file in checkpoint')
    ...
    # construct dataset
    test_dataset, forw_test, back_test = utils.construct_bucket_mean_vb_wc(test_features, test_labels, l_map, c_map,
                                                                           f_map, jd['caseless'])

    test_dataset_loader = [torch.utils.data.DataLoader(tup, 5, shuffle=False, drop_last=False) for tup in test_dataset]
    ...
    if 'f' in args.eva_matrix:
        ...
        test_f1, test_pre, test_rec, test_acc = evaluator.calc_score(ner_model, test_dataset_loader, 4)

        print(test_file + 'test_f1: %.4f test_rec: %.4f test_pre: %.4f test_acc: %.4f\n' % (
